(() => {
  ...
  async function fetchHome() {
    const response = await fetch(`${API_BASE}/admin/assistant/home`, {
      cache: "no-store",
  ...
  function taskMarkup(task) {
    ...
    return `<article class="attention-card ${escapeHtml(priority)}">
      ...
      <a class="attention-action" href="${escapeHtml(href)}">${escapeHtml(action)}</a>
    </article>`;
  }
