...
const pFreq = document.getElementById("p-frequency");
const pTotal = document.getElementById("p-total");

function calc() {
  if (!bArea || !bUrg || !bTotal || !pArea || !pFreq || !pTotal) return;
  ...
  const baseP = 500; // grunnpris plen
  const stepP = 1.2; // kr per m2 over 80 m2
  const aP = Math.max(0, Number(pArea.value) - 80);
  const outP = Math.round((baseP + aP * stepP) * Number(pFreq.value));
  pTotal.textContent = "Estimat: " + outP + " kr";
}
[bArea, bUrg, pArea, pFreq].forEach((el) => el && el.addEventListener("input", calc));
calc();
...
document.querySelectorAll('.book-service-btn').forEach((btn) => {
  btn.addEventListener('click', (e) => {
    ...
    if (serviceId === '3') return openRequestPage('Plenklipping');
    if (serviceId === '4') return openRequestPage('Trefelling');
